...
public class RPGBuilderEditorItemTypeModule : RPGBuilderEditorModule
{
    private Dictionary<int, RPGBItemType> entries = new Dictionary<int, RPGBItemType>();
    private RPGBItemType currentEntry;
    ...
    {
        Dictionary<int, RPGBItemType> dictionary = new Dictionary<int, RPGBItemType>();
        databaseEntries.Clear();
        var allEntries = Resources.LoadAll<RPGBItemType>(RPGBuilderEditor.Instance.EditorData.RPGBDatabasePath + AssetFolderName);
    ...
    public override void CreateNewEntry()
    {
        ...
        currentEntry = CreateInstance<RPGBItemType>();
        RPGBuilderEditor.Instance.CurrentEntry = currentEntry;
    ...
    {
        RPGBItemType entryFile = (RPGBItemType)updatedEntry;
    ...
    public override void DrawView()
    {
        ...
        if (currentEntry.CanBeEquipped)
        {
            currentEntry.EquipType = (EconomyData.EquipFunction)RPGBuilderEditorFields.DrawHorizontalEnum("Equip Type", "",
                (int)currentEntry.EquipType,
                Enum.GetNames(typeof(EconomyData.EquipFunction)));
        }
        else
        {
            currentEntry.ItemTypeFunction = (EconomyData.ItemTypeFunction)RPGBuilderEditorFields.DrawHorizontalEnum("Extra Function", "",
                (int)currentEntry.ItemTypeFunction,
                Enum.GetNames(typeof(EconomyData.ItemTypeFunction)));
        }
