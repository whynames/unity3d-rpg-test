...
public class RPGBCharacterWorldInteraction : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        interactableMask = 1 << GameDatabase.Instance.GetWorldSettings().worldInteractableLayer; 
    }

    private void FixedUpdate()
    {
        Ray ray = cachedCamera.ViewportPointToRay(new Vector3(0.5F, 0.5F, 0));
        ...
        if (hit.transform.gameObject.layer != GameDatabase.Instance.GetWorldSettings().worldInteractableLayer) return;
    ...
    {
        return !GameState.playerEntity.IsInteractiveNodeCasting();
    }
}
