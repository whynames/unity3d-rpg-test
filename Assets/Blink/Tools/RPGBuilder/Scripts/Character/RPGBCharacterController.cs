...
namespace BLINK.RPGBuilder.Characters
{
    public class RPGBCharacterController : MonoBehaviour
    {
        ...
        private CombatEntity thisCombatNode;
        ...
        private void Start()
        {
            if (GameState.playerEntity != null) InitCharacter();
        }
        ...
        private void InitCharacter()
        {
            ...
            thisCombatNode = GetComponent<CombatEntity>();
        ...
        {
            if (UIEvents.Instance.IsPanelOpen("--- DEVELOPER UI ---") && UIEvents.Instance.IsTyping()) return 0;
            switch (name)
            {
                case "Horizontal":
                {
        ...
        private void ClickController()
        {
            ...
            if (!Input.GetMouseButton(0) || UIEvents.Instance.CursorHoverUI || cachedGroundCasting) return;
            RaycastHit hit;
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && !UIEvents.Instance.CursorHoverUI)
                leftButtonDown = true;
            if (Input.GetMouseButtonUp(0))
                leftButtonDown = false;
            if (Input.GetMouseButtonDown(1) && !UIEvents.Instance.CursorHoverUI)
            ...
            if (UIEvents.Instance.CursorHoverUI || !mouseLook ||
