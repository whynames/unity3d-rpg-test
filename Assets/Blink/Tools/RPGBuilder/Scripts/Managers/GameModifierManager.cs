...
namespace BLINK.RPGBuilder.Managers
{
    public class GameModifierManager : MonoBehaviour
    {
        ...
        {
            if (!GameDatabase.Instance.GetWorldSettings().useGameModifiers) return currentValue;
            ...
            foreach (var gameModifier in Character.Instance.CharacterData.GameModifiers)
            {
                ...
                RPGGameModifier gameModifierRef = GameDatabase.Instance.GetGameModifiers()[gameModifier.ID];
        ...
        {
            if (!GameDatabase.Instance.GetWorldSettings().useGameModifiers) return false;
            ReturnedGameModifierValueData newReturnedGameModifierValueData = new ReturnedGameModifierValueData();
            foreach (var gameModifier in Character.Instance.CharacterData.GameModifiers)
            {
                if(!gameModifier.On) continue;
        ...
        {
            if (!GameDatabase.Instance.GetWorldSettings().useGameModifiers) return -1;
            foreach (var gameModifier in Character.Instance.CharacterData.GameModifiers)
            {
                if(!gameModifier.On) continue;
                RPGGameModifier gameModifierRef = GameDatabase.Instance.GetGameModifiers()[gameModifier.ID];
        ...
        public CombatData.CombatEntityStat GetStatValueAfterGameModifier(string gameModifierIdentifier, CombatData.CombatEntityStat statData, int entryID, bool isPlayer)
        {
            if (!GameDatabase.Instance.GetWorldSettings().useGameModifiers) return statData;
            ...
            if (isOverride)
            {
                statData.currentValue = addedAmountFlat;
                statData.currentMinValue = addedMinAmountFlat;
                statData.currentMaxValue = addedMaxAmountFlat;
            }
            else
            {
                statData.currentValue += addedAmountFlat;
