...
namespace BLINK.Controller
{
	public class CharacterAnimator : MonoBehaviour
	{
		...
		public void UpdateState(bool hasMovementRestriction)
		{
			...
			if (GameState.playerEntity.GetMountAnimator() != null)
			{
				if (hasMovementRestriction)
				{
					GameState.playerEntity.GetMountAnimator().SetFloat(CharacterAnimatorParamId.HorizontalSpeed, 0);
				}
				else
				{
					float normHorizontalSpeed = rpgbThirdPersonController.HorizontalVelocity.magnitude /
					                            rpgbThirdPersonController.MovementSettings.MaxHorizontalSpeed;
					GameState.playerEntity.GetMountAnimator().SetFloat(CharacterAnimatorParamId.HorizontalSpeed, normHorizontalSpeed);
				}
				
				GameState.playerEntity.GetMountAnimator().SetFloat(CharacterAnimatorParamId.VerticalSpeed, normVerticalSpeed);
				GameState.playerEntity.GetMountAnimator().SetBool(CharacterAnimatorParamId.IsGrounded, rpgbThirdPersonController.IsGrounded);
			}
		}
	}
}
